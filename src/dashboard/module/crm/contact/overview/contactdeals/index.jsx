...
const ContactDealList = ({ onEdit, deleteDeal, contact }) => {
  ...
  const deals = allDeals.filter(deal => deal.contact_id === contact?.id);
  ...
  const filteredDeals = React.useMemo(() => {
    ...
    switch (filterStatus) {
      case 'won':
        return contactDeals.filter(deal => deal.is_won === true);
      case 'lost':
        return contactDeals.filter(deal => deal.is_won === false);
      case 'pending':
        return contactDeals.filter(deal => deal.is_won === null);
      default:
        return contactDeals;
    }
  }, [deals, filterStatus, contact]);
...
export default ContactDealList;
